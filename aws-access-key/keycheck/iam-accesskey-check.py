...
class AccessKeyAlert:
    def __init__(self):
        self.jira_ticket_url = None
        self.jira_exists = None
        ...
        self.dynamo_client = boto3.session.Session(profile_name=config["aws_dynamodb_profile_name"],
                                                   region_name=config["aws_dynamodb_region"]).client("dynamodb")
        ...
        if config.get("expire_age") == '' or config.get("expire_age") is None:
            config["expire_age"] = 180

        logging.info("Program configs are: Expire_age={}, Slack={}, dump_metric_to_console={}, Jira={}, "
                     "Disabling_expired_keys={}".format(config["expire_age"], config["slack"],
                                                        config["dump_metric_to_console"],
                                                        config["jira_ticket"],
                                                        config["disable_key"]))
...
For more information visit aws.amazon.com/blogs/security/how-to-rotate-access-keys-for-iam-users/
    ...
    def run(self):
        ...
        # 1000 is the maximum number of IAM users that an AWS profile can hold. This way, we are guaranteed to get
        # all users.
        ...
        for user in iam_users_obj['Users']:
            user_name = user['UserName']  # fetching the username so we can pass it into the IAM api
            ...
            for key in access_keys:  # 2 keys to check
                ...
                logging.info(
                    " age for {}'s key with ID {} is {} days old".format(user_name, key["AccessKeyId"], age_in_days))

                self.jira_exists = self.does_jira_exist(
                    key_id=key["AccessKeyId"])  # if a jira ticket c, then we can update the global variable

                if age_in_days >= config["expire_age"] - 15:

                    if config['jira_ticket']:
                        if self.jira_exists:  # if we find a jira ticket for this key id, then we don't want to
                            # create a new jira
                            logging.info(" Jira ticket already exists for {} with key ID {}. {}".format(user_name, key[
                                "AccessKeyId"], self.jira_ticket_url))
                        else:
                            # we want to create a jira ticket for the users which have expired access keys, only if
                            # there already does not exist one for this key
                            self.create_jira_ticket(user_name=user_name, key_id=key["AccessKeyId"],
                    ...
                    if config['slack'] and self.last_message_age(key_id=key["AccessKeyId"]) > 24:
                        ...
                        self.send_slack_message(user_name=user_name, key_age=age_in_days, key_id=key["AccessKeyId"],
                                                expiry_date=expiry_date)

                # conditions for the users with expired access keys
                if age_in_days >= config["expire_age"]:
                    if config['disable_key']:
                        # expired access keys need to be disabled, only the access keys which are in the slackID
                        # mapping (we don't want to touch prod keys)
                        if user_name in mapping:
                            self.disable_access_key(user_name=user_name, key_id=key["AccessKeyId"])

                if config['dump_metric_to_console']:
                    # we want to dump a metric to console for telegraf regardless of if the user's access key is
                    # expiring or not
                    self.dump_metric_to_console(user_name=user_name, key_age=age_in_days, key_id=key["AccessKeyId"])
    ...
    def send_slack_message(self, user_name, key_age, key_id, expiry_date):
        ...
        if mapping.get(user_name) is None:
            return
            ...
            payload = {
                "text": "Your AWS Access Key with ID ending in ******{}, in `{}` is {} day(s) old. It will be expired "
                        "as of {}. For Account Alias `{}` \n {} \n {}".format(key_id[-5:], user_name, key_age,
                                                                              ...
                                                                              self.key_rotation_message(user_name,
                                                                                                        key_id),
                                                                              self.jira_ticket_url),
                'as_user': "true",
        ...
        # after we send a slack message, we want to update the dynamodb entry for the last time we sent a message for
        # this particular aws-access-key
        ping_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        item = {
            "iam_access_key_id": {"S": key_id},
            "last_pinged_time": {"S": ping_time}
        }
        # we first want to make sure that a table exists
        if self.dynamo_client.list_tables().get('TableNames') is None or (
                "iam_access_key_alert" not in self.dynamo_client.list_tables()['TableNames']):
    ...
    def last_message_age(self, key_id):
        results = self.dynamo_client.query(TableName="iam_access_key_alert",
                                           KeyConditionExpression='iam_access_key_id = :key_id',
                                           ExpressionAttributeValues={
                                               ':key_id': {'S': key_id}
                                           }
                                           ).get("Items")

        # if we cannot find the last time we pinged a user with this key ID, we return 30 hours to ensure that the
        # slack message will send
        if not results:
            return 30

        last_messaged_time = results[-1]["last_pinged_time"]["S"]  # fetch the time as a string
        message_age = (datetime.datetime.now() - datetime.datetime.strptime(last_messaged_time,
                                                                            '%Y-%m-%d %H:%M:%S')).total_seconds() // 3600  # convert the datetime obj to hours

        return message_age

    def dump_metric_to_console(self, user_name, key_age, key_id):
        # evaluate the status of the key
        if key_age < config["expire_age"] - 15:
            status = "ok"
        elif key_age < config["expire_age"]:
            status = "expiring"
        else:
        ...
        influx_metric = 'iam,name={},key={},status={},accountAlias={} age={}'.format(user_name, key_id[-5:], status,
                                                                                     self.accountAlias, key_age)
        print(influx_metric)
    ...
    def create_jira_ticket(self, user_name, key_id, expiry_date):
        ...
        payload = {
            "fields": {
                ...
                "description": "AWS Access Key must be rotated for {} with Key ID ending in ****{}. This key is set "
                               "to expire on {}. The AccountAlias is {} \n {}".format(
                    user_name, key_id[-5:], expiry_date, self.accountAlias, self.key_rotation_string),
        ...
        jira_ticket = requests.request("POST", url=url, json=payload, headers=header,
                                       auth=(config["atlassian_api_email"], self.ATLASSIAN_API_TOKEN))
        ...
        if jira_ticket.ok:
            ...
            logging.info(
                " Jira ticket created for user {} with Key ID {}: {}".format(user_name, key_id, self.jira_ticket_url))
    ...
    def does_jira_exist(self, key_id):
        # we check for if a jira already exists for this access key - we don't want to spam the users

        jira_options = {'server': config["jira_server_address"]}
        jira_instance = JIRA(options=jira_options, basic_auth=(
            config["atlassian_api_email"], self.ATLASSIAN_API_TOKEN))  # using the jira sdk for python

        jql_query = 'status not in (Closed, Cancelled,  Resolved, "In Progress") and description~"{}" and ' \
                    'labels=AWS-User-Key-Rotation'.format(key_id[-5:])

        issues_in_ops = jira_instance.search_issues(jql_str=jql_query,
                                                    maxResults=5)  # query looks for any tickets that have this
        # access key ID in it and are open

        # if we find a ticket regarding this expiring key id, then we change the global jira ticket url, otherwise we
        # return false
        if not issues_in_ops:
            return False
        else:
